...
- ⚠️ **`vercel env pull` 不导出加密 secret 真实值**：`KV_*`、`CRON_SECRET`、`COOKIE_SECRET`、`GITHUB_TOKEN` 等拉到本地 `.env` 都是空——本地连不上生产 Redis、也拿不到 CRON 鉴权。需连 Redis 时走 Upstash 控制台或部署到 Vercel 运行时。另：**生产 Redis 经 Vercel KV 集成接入（实际变量 `KV_REST_API_URL` / `KV_REST_API_TOKEN`）**，`stores/redis-client.ts` 的 `getRedis()`（全项目唯一 Redis 实例化点）同时兼容 `UPSTASH_*` 前缀，任一存在即可。
...
业务逻辑采用四层架构：API 路由（HTTP 薄层）→ `services/`（应用服务 / 业务编排）→ `stores/` + `services/interfaces.ts`（端口抽象）→ `stores/*.store.ts`（域级存储实现）+ `stores/redis-client.ts`（Redis 单例）。`/api/match` 的 Agent 编排按**六模块边界**分层（Perception / Memory / Planning / Tools / Orchestration / Observability，定义见 `.agents/skills/walker-northstar/references/planning/agent-six-modules-architecture.md`），`NeedCase` 是核心业务对象（取代旧 DemandEvent）。`docs/adr/ADR-0001` 记录了从 `src/lib` + `src/data` 按职责拆分为 knowledge/profiles/agent/conversation/shared 的历史起点；此后新增 `services/`（应用服务层）和 `stores/`（数据仓储端口层）。**域级 Store 拆分**（2026-06）：原 `conversation/store.ts`（1940 行 God Module）已拆分为 20+ 个域级 Store 文件（match / admin / content / northstar 四域），每个 Store 自包含 key 构造 + 内存降级 + Redis CRUD，`conversation/store.ts` 降为 re-export shim + 跨域聚合（`getNeedCaseStats`）。Astro 展示层文件（pages/、layouts/、components/、scripts/、styles/）保持原位不动。
...
- **`insight.ts`**：需求洞察（工厂模式，`createInsightService({ needCaseStore, topicStore })`）。从 NeedCase 需求事件中聚类，生成 `TopicCandidate`（选题候选，含 clusterKey、density、roleDistribution、优先级）。批处理接口 `processPendingNeedCases()`：同簇合并、回写 NeedCase `topicCandidateId`。通过 Port 注入 NeedCase + Topic store，不直穿 `conversation/store`。
...
- **`ports.ts`**：数据层端口。定义 `AuthState`（`public`/`user`/`admin`）、`UserAccount`（用户名+scrypt 密码哈希，零 PII）/`AccountRepositoryPort`、`UserSession`/`SessionRepositoryPort`、`NeedCase`（核心业务对象，带 `username` 归属）、`MatchSession`、`ConversationMessage`、`MatchFeedbackType`（8 种）、`TopicCandidate`/`TopicCandidateRepositoryPort`（新增 Port，`findAll`/`findById`/`findByClusterKey`/`updateStatus`/`saveAll`）、`InviteCode`、`UserProfile`（按 username）、`Incident`、`PublicStats`、`WorkItem`（后台决策聚合根：evidenceRefs/decision/actions/outcomes/history，状态机 proposal→...→resolved，来源队列 user-demand/walker-thesis/system-event/ai-asset）/`WorkItemRepositoryPort`、`ContentFeedbackEvent`（内容阅读反馈，signal useful/needs-more/outdated）/`ContentFeedbackRepositoryPort`、`AssetKind`/`AssetLifecycleStage`/`normalizeAssetStage`/`AssetEvidenceLink`/`AssetEvidenceLinkRepositoryPort`（资产生命周期统一映射 + 晋升证据链）、`LearningRequest`/`LearningRequestRepositoryPort`（证据不足补证任务）、`SupportConfig`/`SupportConfigRepositoryPort`、`NorthStarOrderRepositoryPort`、`RateLimitPort`/`RateLimitCheckResult`（限流端口，`checkAndIncrement(key, windowSec, limit)`）等接口及对应 RepositoryPort；`MatchSessionRepositoryPort` 含 `createSessionId`/`saveMessages`/`incrementStats`（会话生命周期收口）。上层只依赖这些接口。
- **`redis-client.ts`**：全项目唯一 Redis 实例化点。`getRedis()`（兼容 `UPSTASH_*` / `KV_*` 前缀，缓存单例）+ `__setCachedRedisForTesting`（测试注入）。所有 store 文件 import 此模块，不再各自 `new Redis()`。
- **`memory-fallback.ts`**：统一内存降级工厂 `createMemoryStore<T>()`，为无 Redis 环境提供 Map 存储。
- **match 域 Store**（热路径，Redis `match:*`）：**`match-session.store.ts`**（`MatchSession` + `ConversationMessage`，`match:session:*` / `match:sessions` / `match:messages:*`，含 `incrementMatchStats`）/ **`need-case.store.ts`**（`NeedCase` CRUD + 3 个 List 索引，`match:needcase:*`）/ **`feedback.store.ts`**（`MatchFeedbackEvent`，`match:feedback:*`）/ **`user-profile.store.ts`**（`UserProfile`，`match:profile*`）。
- **admin 域 Store**（管理平面）：**`account.store.ts`**（`auth:account:*` + SETNX 占名）/ **`session.store.ts`**（`auth:session:*` + user→session 索引）/ **`managed-invite-code.store.ts`**（`auth:invite-code:*`）/ **`invite-code.store.ts`**（env 种子 + `auth:invite-code:usage:*` 用量）/ **`work-item.store.ts`**（`admin:workitem:*` + 活跃列表 + 状态索引）/ **`incident.store.ts`**（`admin:incident:*`）/ **`object-grant.store.ts`**（`object-grant:*`）/ **`action-audit.store.ts`**（`action-audit:*`）。
- **content 域 Store**：**`content-feedback.store.ts`**（`content-feedback:*` + 按内容/选题索引）/ **`content-telemetry.store.ts`**（`telemetry:*`）/ **`topic.store.ts`**（`match:topic:*` / `match:topics`，实现 `TopicCandidateRepositoryPort`）/ **`search.store.ts`**（`search:misses`）/ **`experience-event.store.ts`**（`match:experience:*`）/ **`rule-candidate.store.ts`**（`match:rule:*`）/ **`skill-candidate.store.ts`**（`match:skill:*`）/ **`asset-evidence-link.store.ts`**（`asset-link:*` + 单调 seq）/ **`learning-request.store.ts`**（`learning-request:*` + 按状态索引）。
- **northstar 域 Store**：**`northstar.store.ts`**（`northstar:*`，订单/支付/退款/Offer）/ **`support-config.store.ts`**（`northstar:support-config`，赞赏配置）。
- **`rate-limit.store.ts`**：限流仓储（实现 `RateLimitPort`）。Redis INCR+EXPIRE 滑动窗口 + 内存降级。被 `/api/match` 使用（日/分钟/全局三级限流）。
- **`like.store.ts`**：点赞计数仓储。`LikeStore` 接口 + `InMemoryLikeStore`（Map 计数 + 时间戳冷却）+ `UpstashLikeStore`（Redis `incr` 原子自增、`set ex` 冷却，运行时异常降级内存）+ `createLikeStore()` 工厂（有 KV/UPSTASH 凭据用 Upstash，否则内存）。**0 起步真实计数，无写死假基数**。被 `/api/like` 调用。Redis 客户端复用 `redis-client.ts`。
- 以上所有 Store 文件**自包含**（key 构造 + 内存降级 Map + Redis CRUD），通过工厂函数（`createXxxStore()`）创建 Port 实现供 Service 注入。

#### conversation/（re-export shim + 跨域聚合）

- **`store.ts`**：**已降为 re-export shim**（原 1940 行 God Module，域级 Store 拆分后仅 ~300 行）。重新导出所有域 Store 的函数和类型（保持上层 import 路径兼容），唯一本地保留的是 `getNeedCaseStats()`（跨域统计聚合：读 match 域 need-case + feedback → 按分类/摩擦层/能力/反馈/审查状态聚合 + 日趋势）。被 MCP server 和 `/api/match-feedback`（`saveMatchFeedback`）通过 re-export 引用。**新代码应直接从域 Store import，不经过此文件。**
