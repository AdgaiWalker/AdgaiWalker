...
测试基于 Vitest（`vitest.config.ts`），测试文件位于 `src/services/*.test.ts`（`matching.service.test.ts`、`visibility.service.test.ts`、`invite.service.test.ts`）。补充验证依赖 `npm run build`（构建通过）和 `npx astro check`（类型检查）。
...
| 路径 | 功能 | 认证 |
|------|------|------|
| `/api/match` | 用户需求匹配（HTTP 薄层 → `questionService` → 本地匹配 + AI Gateway） | 无（IP 限流） |
| `/api/match-end` | 结束匹配会话 | 无（sessionId） |
| `/api/match-feedback` | 推荐结果反馈（`resolved`/`stuck`/`not-fit`/`want-tutorial`/`first-draft`/`next-step-clear`/`wrong-direction`/`need-tutorial`） | 无 |
...
| `/api/admin/invite-verify` | 邀请码验证准入（管理员 cookie 直接放行，否则校验邀请码） | 无（邀请码） |
...
- **`ToolMatchChat.astro`**（`tools/`）：工具匹配对话组件，提供 `/api/match` 匹配会话的前端 UI。
...
业务逻辑采用四层架构：API 路由（HTTP 薄层）→ `services/`（应用服务 / 业务编排）→ `stores/` + `services/interfaces.ts`（端口抽象）→ `conversation/store.ts`（存储实现）。`docs/adr/ADR-0001` 记录了从 `src/lib` + `src/data` 按职责拆分为 knowledge/profiles/agent/conversation/shared 的历史起点；此后又新增 `services/`（应用服务层）、`stores/`（数据仓储端口层）和 `infrastructure/`（基础设施层，当前为预留空目录）。Astro 展示层文件（pages/、layouts/、components/、scripts/、styles/）保持原位不动。
...
- **`match.ts`**：本地匹配引擎，导出 `matchSiteResources()`。接收用户需求，基于关键词评分 + 站内资源索引进行本地初筛，输出 `MatchResult`（分类、资源、串联语、判断结论、摩擦层、能力方向）。不依赖 AI API，被 `question.service.ts` 调用，可独立运行。
...
- **`interfaces.ts`**：四层架构的业务层端口。定义 `MatchingServicePort`、`VisibilityServicePort`（角色 `public`/`invited`/`admin` × 可见性 `public`/`draft`/`private`/`admin-only`）、`InviteServicePort`、`QuestionServicePort`。
- **`question.service.ts`**：提问处理编排，`/api/match` 的业务核心。清洗消息 → 本地匹配（`matchSiteResources`）→ 按需调用 AI Gateway（`callGateway`）→ 持久化会话与需求事件。HTTP 逻辑（速率限制、请求解析）留在 API 层。
- **`matching.service.ts`**：本地匹配服务（`matchNeed`）。
- **`visibility.service.ts`**：可见性判断（`canSee`、`redactDemandEvent`、`filterStats`）。
- **`invite.service.ts`**：邀请码验证准入（`verifyAndAdmit`）。

#### stores/（数据仓储端口层）

- **`ports.ts`**：四层架构的数据层端口。定义 `DemandEvent`、`MatchSession`、`MatchFeedbackType`（8 种）、`InviteCode`、`UserProfile`、`TopicCandidate`、`PublicStats` 等接口及对应 RepositoryPort，上层只依赖这些接口。
- **`match-session.store.ts` / `demand-event.store.ts` / `feedback.store.ts` / `invite-code.store.ts`**：各仓储的工厂实现，**委托给 `conversation/store.ts`** 保持兼容。

#### conversation/（对话存储实现层）

- **`store.ts`**：会话与事件存储的 Redis + 内存降级实现。管理 `MatchSession`、`DemandEvent`、`TopicCandidate`、`MatchFeedbackEvent`、`ConversationMessage` 生命周期，导出 `getRedis()`、`createSessionId()`、`saveConversationMessages()`、`getDemandStats()`、`getTopicCandidates()` 等。被 `stores/` 仓储委托调用，也被 MCP server（`getDemandStats`）和 `/api/match-feedback`（`saveMatchFeedback`）直接引用。
