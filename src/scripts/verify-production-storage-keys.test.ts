...
describe('verify-production-storage 键名与生产 store.ts 一致', () => {
  ...
  it('WorkItem 实体 / 全量 / active 索引键名一致', () => {
    ...
    expect(script).toContain('`match:workitem:${workItemId}`');
    expect(script).toContain("'match:workitems'");
    expect(script).toContain("'match:workitems:active'");
    // 与生产真相源逐字对账。
    expect(WORKITEM_REDIS_KEYS.workItem(sampleId)).toBe(`match:workitem:${sampleId}`);
    expect(WORKITEM_REDIS_KEYS.list).toBe('match:workitems');
    expect(WORKITEM_REDIS_KEYS.activeList).toBe('match:workitems:active');
  });
...
describe('verify-production-storage-local（本地 Redis 等价版）键名与生产 store.ts 一致', () => {
  ...
  it('从生产真相源派生键名，且 ContentFeedback 不含 match: 前缀', () => {
    ...
    expect(script).toContain('`match:workitem:${id}`');
    expect(script).toContain("'match:workitems'");
    expect(script).toContain("'match:workitems:active'");
