...
export function SearchModalIsland(): ReactElement {
  ...
  const [status, setStatus] = useState('输入关键词搜索...');
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      if (!value) {
        setResults([]);
        setStatus(pagefindRef.current ? '输入关键词搜索...' : '搜索索引加载中...');
        return;
      }
      ...
      setStatus(resultData.length === 0 ? `没有找到「${value}」相关记录` : '');
    }, 200);
  ...
  return (
    <div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-label="站内搜索">
      ...
      <div className="relative z-10 max-w-2xl mx-auto mt-[15vh] px-4">
        <div className="rounded-2xl border border-border bg-bg shadow-2xl shadow-black/10 overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
            <svg className="w-5 h-5 text-text-muted shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <input
              ...
              placeholder="搜索..."
              className="flex-1 bg-transparent border-none outline-none text-text text-base font-body placeholder:text-text-muted"
              autoComplete="off"
            />
            <button
              type="button"
              className="text-text-muted hover:text-text text-xs px-2 py-1 rounded border border-border cursor-pointer bg-transparent font-mono"
              onClick={() => setOpen(false)}
            >
              ESC
            </button>
          ...
          <div className="max-h-[50vh] overflow-y-auto px-2 py-2">
            ...
            {results.map((result) => (
              <a
                key={result.url}
                href={result.url}
                className="block px-4 py-3 rounded-lg hover:bg-bg-subtle transition-colors no-underline group"
              >
                <div className="text-text text-sm font-heading group-hover:text-gold transition-colors mb-1">
                  {result.title}
                </div>
                <p className="text-text-muted text-xs line-clamp-2">{result.excerpt}</p>
              </a>
            ))}
          </div>
        </div>
