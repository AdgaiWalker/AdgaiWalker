---
name: 极客常用交流群 (QQ群)
description: 关于 Antigravity 切号插件官方群、廉价 AI 探索群、Codex API 中转与低价 Gemini 账号售卖交流群。
category: community
tags: [AI, 编程, 中转, QQ群]
rating: 5
url: https://lcndccjmtf4f.feishu.cn/wiki/GEZ2wYIwSiE8BNkJON9ccDsHnMg
published: true
communities:
  - name: Antigravity 官方群 (1025957317)
    description: 关于 Antigravity 切号插件官方群，AI 编程技术交流与疑问解答。
    qrCode: /images/1025957317.png
    badge: 插件官方
    tag: "1025957317"
  - name: 廉价 AI 探索群 (644241363)
    description: AI 编程与工具提效，不局限于 Agent 工具，极致挖掘最高性价比的 AI 开发方案。一旦有群友成功探索，就会用上最高性价比的 AI。
    qrCode: /images/644241363.png
    badge: 极客探索
    tag: "644241363"
  - name: Codex 中转群 (874615899)
    description: Codex 中转推荐与使用交流，秋知目前正在稳定使用的一家 API 中转方案。
    qrCode: /images/874615899.png
    badge: API中转
    tag: "874615899"
  - name: 廉价 Gemini Pro 群 (921917596)
    description: 目前见到过的最低价 Gemini Pro 账号售卖交流群。只保证 Gemini 可用，不保证 Antigravity 等其他工具能用。
    qrCode: /images/921917596.png
    badge: 账号售卖
    tag: "921917596"
---
