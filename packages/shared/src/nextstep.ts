...
export interface NextStepBucket {
  id: NextStepBucketId;
  /** 面向访客的中文语义标签：仅用于「依据」展示，不替代 nextStep 正文 */
  label: string;
  triggers: string[];
  nextStep: string;
}

const BUCKETS: NextStepBucket[] = [
  {
    id: 'learn-ai',
    label: '学 AI 入门',
...
export function listNextStepBuckets(): readonly NextStepBucket[] {
  return BUCKETS;
}

/** 桶 id → 面向访客的中文语义（未知 id 落到 default，不抛错） */
export function nextStepBucketLabel(id: NextStepBucketId): string {
  const bucket = BUCKETS.find((b) => b.id === id) ?? BUCKETS[BUCKETS.length - 1];
  return bucket.label;
}

/**
 * 命中该描述的具体触发词（结果页「依据」展示用）。
 * 与 matchNextStepBucket 同序同源，保证展示的依据与规则版结论一致。
 */
export function matchedNextStepTrigger(body: string): {
  bucketId: NextStepBucketId;
  trigger: string;
} | null {
  const lower = body.toLowerCase();
  for (const bucket of BUCKETS) {
    if (bucket.id === 'default') continue;
    const trigger = bucket.triggers.find((t) => lower.includes(t));
    if (trigger) return { bucketId: bucket.id, trigger };
  }
  return null;
}
