...
### T4.D 悬浮窗批（2026-09-05 追加，用户拍板：对话直接进主页，开悬浮窗）

设计决策：对话是伴生行为（边逛边问），跳页是流失点；悬浮窗为主、`/ask` 独立页保留为直链兜底（搜索升级落点/可分享）。伴生卡（名片语义）与悬浮按钮（入口语义）并存。

- [ ] T4.D1 悬浮按钮（AppShell 全局挂载）：右下角圆形按钮「● 小影」+ 呼吸点；默认低调（半透明，hover 实）；点击开/关悬浮窗；移动端 media query 缩小
  - 挂载点：`AppShell.tsx`（全局壳层，全站可用——文章页读到一半问「这篇讲的啥」是最自然场景）
  - 排除页：`/ask` 自身（同页已有完整对话，不叠窗）
- [ ] T4.D2 悬浮窗组件 `AssistantFloating.tsx`：约 380×520、右下锚定、Portal 渲染到 body；窄栏（● 小影 · duola 的管家 + 关闭）+ 对话流（含空态 3 示例卡）+ 输入条
  - 状态：open/close 组件态；悬浮窗与 /ask 会话各自独立（刷新即新会话，行为一致）
  - 自决：可拖动窗体若与页面滚动冲突，第一版砍掉拖动（右下锚定已够用），记日志
- [ ] T4.D3 重构抽离：`AssistantPanel` 拆为 `AssistantThread`（消息流+空态示例+输入条，纯展示，两壳共用）+ 两个壳（/ask 整页壳 / 悬浮窗壳）；`/ask` 行为不回归（Enter 发送、?q= 预填、重新开始）
- [ ] T4.D4 移动端适配：窗体近全宽底部弹出（CSS media query）；悬浮按钮避开移动端底部导航
- [ ] T4.D5 a11y：悬浮窗 `role=dialog` + `aria-label`；Esc 关闭；打开焦点入输入框、关闭还焦点给按钮（沿用 SearchModal 焦点管理模式）
- [ ] T4.D6 验收门：主页/文章页/资源页均可唤起且独立对话；`/ask` 页无悬浮按钮；Esc/关闭按钮/再点按钮三种关闭路径；typecheck + test:web + build:web + verify:geo 绿；浏览器实测留证（含 390px 移动宽度模拟）
