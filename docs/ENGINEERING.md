...
- 旧兼容路径（`/condition` `/kit` `/showcase` `/ideas` `/projects` `/content` `/ideas/new`）走 **301**；点子 / 项目进 `/explore` 对应视图，禁止先吐首页再靠客户端跳
...
- `/explore` `/learn` `/gear` `/advance` `/projects/ferry` 有独立内容，进 sitemap；`/tools` `/support` `/login` `/exchange` 保持 `noindex`。
