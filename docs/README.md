...
`docs/` 不是当前项目工作台。当前有效的 PRD、plan、todo、architecture、current-state 统一放在：
...
3. **额外材料**：专题、素材、图谱、媒体、非当前调用资料，放入 `docs/AI赋能/`、`docs/media/` 或后续明确命名的资料目录。
...
| `docs/adr/` | 长期决策记录，解释为什么做某个架构选择。 | 保留。 |
...
| `hai-stack/README.md` | `hai-stack` 这套 skill 方法论自己的说明书。 | 保留，属于子系统入口。 |
...
| `project-docs-index.md` + `creator-system-current-state.md` | 不合并 | 一个是决策入口，一个是真实状态；入口不能变成状态长文。 |
