...
11. 提问门槛 2 字符（`isValidAssistantBody`，前后端同源），与卡口 4 字规则解耦——对话语境中文两字即成句（2026-09-03 上线后修订）。
...
| 切片 | 范围 | 状态 |
|---|---|---|
| 阶段 0 | harness 本机跑通：pnpm install/build → 冒烟 → 凭据 → 内存实测（224MB） | ✅ 2026-08-30 |
| 阶段 1 | Run 合同 + AssistantRunnerPort + 双 Adapter + `/api/assistant` + 单测 + 本地 e2e | ✅ 2026-08-30 |
| 阶段 2 | web 对话框面板 + 关于页入口 + 搜索无结果升级 | ✅ 2026-08-30 |
| 阶段 3 | 公网部署（盒子）+ 生产验收 + 切流（`api.iwalk.pro` 上线，验证盒 2026-09-03 起算） | ✅ 2026-09-03 |

上线后修订（2026-09-03）：提问门槛降为 2 字符（见决策锚点 11）；盒子运维通道增加腾讯云 TAT「执行命令」（SSH 熔断时的带外替代）。执行明细与事故记录见 `TODO-SITE-ASSISTANT.md`。
