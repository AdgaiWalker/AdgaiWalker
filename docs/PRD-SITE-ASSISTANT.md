...
> **2026-09-11 本地首页桌宠（用户授权）**：小影是卡片上方可拖拽的生物。心思用同一朵云朵气泡体现：平时没有泡；醒着被点到或进页时突然冒出一朵；同一朵里听你说、再变成它的一句。不铺检索列表。其它页仍开 SearchModal。不改变助手端点、引用规则与权限边界。未接音效或承接道具；未发布生产。

> 3D 运行时按可见性加载，离屏/后台/手动暂停或系统减少动态效果时停止动画循环，离开首页释放 GPU 资源。模型加载或 WebGL 失败使用静态形象，搜索输入继续可用。资源和代码说明见 `public/xiaoying/README.md`。
...
| 首个证明点 | 访客在对话框问「Dora 是谁」，得到正确且引用站内文章的回答 |
...
2. 三个入口：① 对话框（核心交付）；② 搜索无结果时升级（普通搜索保留不动）；③ 推荐（不独立做，即助手回答中的自然引用）。
   **2026-09-06 修订（用户拍板）**：普通搜索与智能搜索**合并为同一个「搜索或问小影」面板**——搜得到列结果，搜不到 / 回车 / 点「问问小影」都在面板内直接提问（不跳页）。入口面收敛为：右下角常驻搜索框（AskBar，全站唯一搜索/提问入口）+ `/ask` 独立页（直链兜底）；原悬浮聊天球、主页小影卡、顶栏/侧栏/移动栏搜索按钮、双入口「问小影」按钮全部移除。②的「搜索无结果升级」由面板内「问问小影」行承接。
...
| 用户 | 场景 | 期待 |
|---|---|---|
| 想了解站主的访客 | 「Dora 是谁」「他为什么用 Mac」 | 一句话得到基于站内证据的回答 |
...
- web 对话框：右下 AskBar 合并搜索面板 + 独立页 `/ask`（直链兜底）；无结果/回车/「问问小影」在面板内提问，不跳页；
...
- 文章权限层、账号体系、登录；
- 独立推荐系统（推荐 = 助手回答的自然引用）；
- 全局悬浮聊天窗（入口收敛：对话框有明确入口，不到处跟着；2026-09-06 起由右下角搜索框 + 合并搜索面板替代，见 §2.1 修订）；
...
搜索无结果升级：普通搜索与提问同一面板；`hadResults=false` / 回车 / 点「问问小影」都在面板内直接提问，不跳 `/ask`。
...
| AI 开（质量） | 本地 e2e：「Dora 是谁」「想学 AI 从哪开始」回答正确、引用站内文章且 citations ⊆ citable（文章正文可能仍写 duola） |
...
上线后修订（2026-09-03）：提问门槛降为 2 字符（见决策锚点 11）；盒子运维通道增加腾讯云 TAT「执行命令」（SSH 熔断时的带外替代）。执行明细与事故记录见 `archive/TODO-SITE-ASSISTANT.md`（已归档，禁止当待办）。
