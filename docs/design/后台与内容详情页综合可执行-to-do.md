...
| P5 NorthStar 与经营系统 | 隔离护栏已建（第十六轮 northstar-containment 不变量测试，锁死 draft/private 永不进公开发布路径）。**支付/订单/退款 + 公共网络本体仍 Human Gate + 产品定义** | 部分完成（护栏） | 需 Walker 明确经营范围、支付通道与公开边界；个人闭环须保持可独立关闭（已有测试守护） |
...
- [ ] Contributor 授权**策略**（哪些角色映射、哪些具体 grant、7 角色模板）—— 待 Walker 定后写入 ObjectGrant store；机制已就绪可承载。
