...
export class PublicationService {
  ...
  async publishWebsite(workId: string, artifactHash: string) {
    ...
    const publication = await this.publications.upsert({ id: newId(), submissionId: workId, channel: 'WEBSITE', artifactHash, status: 'PUBLISHED', url: `${base}/posts/${encodeURIComponent(saved.slug)}`, publishedAt: new Date() });
    if (this.works.setStatus) await this.works.setStatus(workId, 'PARTIALLY_PUBLISHED', artifactHash);
    return publication;
  }
  ...
  async prepareWechat(workId: string, artifactHash: string) {
    ...
    const value = {
      ...
      fieldChecklist: ['title', 'summary', 'author', 'html', 'landscapeCover', 'portraitCover'],
      artifactHash,
    };
